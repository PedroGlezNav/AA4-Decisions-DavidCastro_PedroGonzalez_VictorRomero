	...
	Graph(std::vector<std::vector<int>> grid_terrain){
		for (int i = 0; i < grid_terrain.size(); i++) {
			for (int j = 0; j < grid_terrain[i].size(); j++) {
				if (grid_terrain[i][j] != 0) {

					Node* newFromNode = new Node(0, Vector2D(j, i));

					//Left
					if (i - 1 > 0 && grid_terrain[i - 1][j] != 0) {
					...
					//Right
					if (i + 1 < grid_terrain.size() && grid_terrain[i + 1][j] != 0) {
					...
					//Down
					if (j + 1 < grid_terrain[i].size() && grid_terrain[i][j + 1] != 0) {
